...
aux::log::log(log_params* params, std::string output_file_path, std::size_t buff_sz) :
...
void aux::log::set_forward(std::ostream* f) {
    do_forward = true;
    forward = f;
}

aux::log::int_type aux::log::overflow(int_type ch)
{
    if (sink_ && ch != EOF)
    {
...
bool aux::log::forward_and_flush()
{
    char* p = pbase();
    *(pptr()) = '\0';
...
aux::logger::logger(logger_params* params, std::string seed_string, std::string output_dir_path) :
...
std::string aux::logger::get_seed_string() {
    return seed_string;
}

#ifndef _WIN32
std::string aux::logger::get_system_info() {
