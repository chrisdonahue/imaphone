...
void aux::audio_util::calculate_fft_buffer_size(unsigned num_frames, unsigned n, unsigned o, unsigned* fft_num_bins, unsigned* fft_num_frames, unsigned* fft_output_buffer_length) {
...
void aux::audio_util::window(const char* type, unsigned n, float* window_buffer) {
...
void aux::audio_util::apply_window(unsigned n, kiss_fft_scalar* buffer, const float* window) {
    for (unsigned i = 0; i < n; i++) {
        buffer[i] *= window[i];
    }
}

void aux::audio_util::fft_real(kiss_fftr_cfg cfg, unsigned num_frames, const float* input, unsigned n, unsigned overlap, const float* window, kiss_fft_scalar* in_buffer, kiss_fft_cpx* out_buffer, double* magnitude, double* phase) {
    ...
    while (num_remaining > 0) {
        ...
        // apply window
        aux::audio_util::apply_window(n, in_buffer, window);
...
void aux::audio_util::find_moving_average(unsigned type, unsigned n, const double* data, double* moving_average_buffer, double* data_average, double* max_deviation_below, double* max_deviation_above, unsigned past_radius, unsigned future_radius, double alpha) {
    // NON-MOVING AVERAGE
    if (type == 0) {
        double sum = 0;
...
void aux::audio_util::fill_time_domain_buffer(unsigned num_samples, double sr, float* buffer) {
    for (unsigned frame = 0; frame < num_samples; frame++) {
        buffer[frame] = float(frame)/sr;
    }
}

void aux::audio_util::fill_frequency_domain_buffer(unsigned fft_size, double sr, float* buffer) {
...
std::string aux::audio_util::float_buffers_to_graph_string(std::string options, std::string x_label, std::string y_label, bool index_as_x_axis, unsigned n, const float* x, const float* y, const float* z) {
	std::stringstream ss;
	ss << options;
	ss << std::endl;
	ss << x_label;
...
std::string aux::audio_util::double_buffers_to_graph_string(std::string options, std::string x_label, std::string y_label, bool index_as_x_axis, unsigned n, const double* x, const double* y, const double* z) {
	std::stringstream ss;
	ss << options;
	ss << std::endl;
	ss << x_label;
...
void aux::audio_util::applyEnvelope(unsigned n, float* buffer, const float* envelope) {
	for (unsigned i = 0; i < n; i++) {
		buffer[i] *= envelope[i];
	}
}
...
void aux::audio_util::followEnvelope(unsigned n, float* buffer, float* envelope, double attack_in_ms, double release_in_ms, double samplerate) {
...
void aux::audio_util::findEnvelope(bool ignoreZeroes, unsigned n, float* wav, float* env) {
...
double aux::audio_util::compareAmplitudes(unsigned numSamples, const float* samplesOne, const float* samplesTwo) {
    double sum = 0;
...
double aux::audio_util::compareAmplitudesWeighted(unsigned numSamples, const float* samplesOne, const float* samplesTwo, float weight) {
    double sum = 0;
    for (unsigned frameNum = 0; frameNum < numSamples; frameNum++) {
        double error = fabs(samplesTwo[frameNum] - samplesOne[frameNum]);
...
void aux::audio_util::FftReal(kiss_fftr_cfg cfg, unsigned numFrames, const float* input, unsigned n, unsigned overlap, const float* window, kiss_fft_scalar* in, kiss_fft_cpx* out, bool dB, double* magnitude, double* phase) {
    float dBref = DBREF;
    ...
    while (numRemaining > 0) {
        ...
        // apply window
        aux::audio_util::apply_window(n, in, window);
