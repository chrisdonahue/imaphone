...
void aux::juce_file_io::get_wav_file_metadata(std::string path, unsigned long* num_frames, unsigned* bits_per_sample, double* length_seconds, double* sampling_frequency, double* nyquist_frequency) {
...
void aux::juce_file_io::load_wav_file(std::string path, unsigned chunk_size, unsigned num_frames, float* buffer) {
    File input(path);
    if (!(input.existsAsFile())) {
        std::cerr << "Invalid input file: " << path << std::endl;
        return;
...
void aux::juce_file_io::save_wav_file(std::string path, std::string desc, std::string origin, double sample_rate, unsigned chunk_size, unsigned num_frames, float* data) {
    File output(path);
...
void aux::juce_file_io::save_text_file(std::string path, std::string text) {
    File output(path);
    if (output.existsAsFile()) {
        output.deleteFile();
    }
...
void aux::juce_file_io::append_to_text_file(std::string path, std::string text) {
    File output(path);
    if (!output.existsAsFile()) {
		output.create();
    }
...
std::string aux::juce_file_io::read_text_from_file(std::string path) {
    File input(path);
    if (!input.existsAsFile()) {
		return "";
    }
